...
namespace Checkers
{
    public abstract class BaseClickComponent : MonoBehaviour, IPointerClickHandler, IPointerEnterHandler, IPointerExitHandler
    {
        ...
        public void ChangeMaterialOnHover(CellComponent component, bool isSelected) // меняем материал при наведении
        {
            if (isSelected)
            {
                component.AddAdditionalMaterial(hoverMaterial, 1);
            }
            else
            {
